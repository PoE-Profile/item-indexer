...
},{"got":4}],45:[function(require,module,exports){
...
Vue.use(vueResource);
Vue.http.headers.common['X-CSRF-TOKEN'] = window.PHP.csrf_token;
...
},{"vue":45,"vue-resource":44}],48:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
    value: true
});
var CompereItemMods = exports.CompereItemMods = function CompereItemMods() {
    var arr = [];
    var selectedChar;
    return {
        ...
        compereItem: function compereItem(item) {
            var characterItem;
            var compereItem = item.stats[0];
            var self = this;
            //get character item with the same type
            $.map(selectedChar, function (value, itemType) {
                if (itemType === item.type) {
                    characterItem = value;
                }
            });

            characterItem.forEach(function (charMod) {
                self.addMod(charMod, true);
            });

            compereItem.forEach(function (srchMod) {
                self.addMod(srchMod);
            });
        },

        addMod: function addMod(mod) {
            var isCharItem = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];

            var name = mod.replace(/\d+/, '');
            var value = parseInt(mod.replace(/\D/g, ''));
            // if There is update stop
            if (this.updateVal(name, value, isCharItem)) {
                return;
            }
            var difference = {};
            difference.name = name;
            difference.value = isCharItem ? value : 0;
            difference.diff = isCharItem ? value * -1 : value;
            difference.color = isCharItem ? 'red' : 'green';
            arr.push(difference);
        },

        // Update Mod Value if updated return's TRUE else False
        updateVal: function updateVal(name, value) {
            var isCharItem = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];

            var returnVal = false;
            arr.forEach(function (item) {
                if (item.name === name) {
                    item.value += isCharItem ? value : 0;
                    item.diff += isCharItem ? value * -1 : value;
                    item.color = isCharItem ? 'red' : 'green';
                    returnVal = true;
                }
            });
            return returnVal;
...
exports.default = {
    components: {
        'v-select': _vueMultiselect2.default
    },

    props: ['bigData', 'uniqNames'],
    ...
    data: function data() {
        ...
        return _ref = {
            typeLineStore: [],
            leagues: ['Essence', 'Hardcore Essence', 'Standard', 'Hardcore Standard'],
            rarityTypes: [{ key: 0, name: 'Normal' }, { key: 1, name: 'Magic' }, { key: 2, name: 'Rare' }, { key: 3, name: 'Unique' }, { key: 4, name: 'Gem' }, { key: 5, name: 'Currency' }],
            isIdentified: [{ key: 0, name: 'Not Identified' }, { key: 1, name: 'Identified' }],
            isCorrupted: [{ key: 0, name: 'Not Corrupted' }, { key: 1, name: 'Corrupted' }],
            itemName: '',
            identified: '',
            corrupted: '',
            rarity: '',
            ...
            green_socket: '',
            red_socket: ''
        }, (0, _defineProperty3.default)(_ref, 'red_socket', ''), (0, _defineProperty3.default)(_ref, 'buyout', ''), (0, _defineProperty3.default)(_ref, 'isLoading', false), (0, _defineProperty3.default)(_ref, 'currentMod', ''), (0, _defineProperty3.default)(_ref, 'uniques', []), _ref;
    },

    computed: {
        'oldFields': function oldFields() {
            return window.PHP.oldFields;
        },

        'allTypes': function allTypes() {
            return this.bigData.TypeAndBaseType.map(function (TypeAndBaseType) {
                return TypeAndBaseType.type;
            });
        }
    },

    ready: function ready() {
        this.uniqNames = JSON.parse(this.uniqNames);
        if ((0, _keys2.default)(this.oldFields).length > 0) {
            this.setOldFields();
        }
    ...
    watch: {
        'type': function type(_type, oldVal) {
            this.typeLineStore = [];
            this.typeLineStore = this.bigData.TypeAndBaseType.filter(function (TypeAndBaseType) {
                return TypeAndBaseType.type === _type;
            })[0].baseTypes;
        }
    },
    ...
    methods: {
        ...
        reset: function reset() {
            ...
            this.links_min = '';
            this.links_max = '';
            this.ex_white_socket = '';
            this.ex_blue_socket = '';
            this.ex_green_socket = '';
            this.ex_red_socket = '';
            this.white_socket = '';
            this.blue_socket = '';
            this.green_socket = '';
        ...
        addMod: function addMod() {
            var newId = this.mods.length > 0 ? this.mods.slice(-1)[0].id + 1 : 0;
            this.mods.push({ id: newId, modId: '', modName: '', min: '', max: '' });
        },

        removeMod: function removeMod(mod) {
            this.mods.$remove(mod);
        },

        setOldFields: function setOldFields() {
            var self = this;
            if ('mod' in this.oldFields) {
                this.oldFields['mod'].forEach(function (oldMod, index) {
                    var modIdz = oldMod.id;
                    var tt = self.bigData.mods.filter(function (dbMod) {
                        return dbMod.id == modIdz;
                    })[0];
                    self.mods.push({ id: index, modId: tt.id, modName: tt.name, max: oldMod.max, min: oldMod.min });
                });
            }
            var oldRarity = this.filterOld(this.rarityTypes, parseInt(this.oldFields.rarity));
            var oldCorupted = this.filterOld(this.isCorrupted, parseInt(this.oldFields.corrupted));
            var oldIdentified = this.filterOld(this.isIdentified, parseInt(this.oldFields.identified));
            this.itemName = this.oldFields.name;
            this.league = this.oldFields.league;
            this.socket_max = this.oldFields.socket_max;
            this.socket_min = this.oldFields.socket_min;
            this.level_max = this.oldFields.level_max;
            this.level_min = this.oldFields.level_min;
            this.links_min = this.oldFields.links_min;
            this.links_max = this.oldFields.links_max;
            this.ex_white_socket = this.oldFields.ex_white_socket;
            this.ex_blue_socket = this.oldFields.ex_blue_socket;
            this.ex_green_socket = this.oldFields.ex_green_socket;
            this.ex_red_socket = this.oldFields.ex_red_socket;
            this.white_socket = this.oldFields.white_socket;
            this.blue_socket = this.oldFields.blue_socket;
            this.green_socket = this.oldFields.green_socket;
            this.red_socket = this.oldFields.red_socket;
            this.character = this.oldFields.character;
            this.buyout = this.oldFields.buyout;
            this.identified = oldIdentified;
            this.corrupted = oldCorupted;
            this.rarity = oldRarity;
            this.typeLine = this.oldFields.typeLine;
            this.type = this.oldFields.type;
        },

        filterOld: function filterOld(arr, find) {
            if (isNaN(find)) {
                return '';
            }
        ...
        updateTypeLine: function updateTypeLine(value) {
            this.typeLine = value;
        },

        updateLeague: function updateLeague(value) {
            this.league = value;
        },

        updateRarity: function updateRarity(value) {
            this.rarity = value;
        },

        updateIdentified: function updateIdentified(value) {
            this.identified = value;
        },

        updateCorrupted: function updateCorrupted(value) {
            this.corrupted = value;
        },

        updateMod: function updateMod(value, id) {
            this.mods[id].modId = value.id;
        },
        ...
        asyncFind: function asyncFind(query) {
            if (query.length === 0) {
                ...
                this.isLoading = true;
                this.uniques = this.uniqNames.filter(function (element, index, array) {
                    return element.toLowerCase().includes(query.toLowerCase());
                });
                if (this.uniques.length === 0) {
                    this.uniques.push(query);
                }
...
;(typeof module.exports === "function"? module.exports.options: module.exports).template = "\n<div class=\"row\" style=\"padding-bottom: 20px;padding-left: 15px;padding-right: 15px\">\n    <label>Name</label>\n    <a href=\"#\" class=\"btn btn-danger btn-sm\" @click.prevent=\"clearName\">clear</a>\n    <v-select :options=\"uniques\" ,=\"\" :selected=\"itemName\" :searchable=\"true\" :loading=\"isLoading\" @search-change=\"asyncFind\" @update=\"updateName\" placeholder=\"Search for Item\" label=\"name\">\n    </v-select>\n    <input type=\"hidden\" name=\"name\" v-model=\"itemName\" class=\"form-control\">\n</div>\n\n<div class=\"row\">\n\n    <div class=\"col-lg-3\">\n        <label>Type</label>\n        <v-select :options=\"allTypes\" ,=\"\" :selected=\"type\" :show-labels=\"false\" @update=\"updateType\" placeholder=\"Select Item Type\" label=\"name\">\n        </v-select>\n        <input type=\"hidden\" name=\"type\" v-model=\"type\">\n    </div>\n\n    <div class=\"col-lg-3\">\n        <label>Base</label>\n        <v-select :options=\"typeLineStore\" ,=\"\" :selected=\"typeLine\" :show-labels=\"false\" @update=\"updateTypeLine\" placeholder=\"Select Base\" label=\"name\">\n        </v-select>\n        <input type=\"hidden\" name=\"typeLine\" v-model=\"typeLine\">\n    </div>\n\n\n    <div class=\"col-lg-3\">\n        <label>Rarity</label>\n        <v-select :options=\"rarityTypes\" ,=\"\" :selected=\"rarity.name\" :show-labels=\"false\" @update=\"updateRarity\" placeholder=\"Select Rarity\" key=\"key\" label=\"name\">\n        </v-select>\n        <input type=\"hidden\" name=\"rarity\" v-model=\"rarity.key\">\n    </div>\n\n\n    <div class=\"col-lg-3\">\n        <label>League</label>\n        <v-select :options=\"leagues\" ,=\"\" :selected=\"league\" :show-labels=\"false\" @update=\"updateLeague\" placeholder=\"Select League\" label=\"name\">\n        </v-select>\n        <input type=\"hidden\" name=\"league\" v-model=\"league\">\n    </div>\n\n</div>\n\n<br>\n\n<div class=\"row\">\n\n    <div class=\"col-lg-4\" style=\"padding-bottom: 30px;\">\n        <label class=\"text-center\">identified</label>\n        <v-select :options=\"isIdentified\" ,=\"\" :selected=\"identified.name\" :show-labels=\"false\" @update=\"updateIdentified\" placeholder=\"Select\" key=\"key\" label=\"name\">\n        </v-select>\n        <input type=\"hidden\" name=\"identified\" v-model=\"identified.key\">\n    </div>\n\n    <div class=\"col-lg-4\" style=\"padding-bottom: 30px;\">\n        <label>Corrupted</label>\n        <v-select :options=\"isCorrupted\" ,=\"\" :selected=\"corrupted.name\" :show-labels=\"false\" @update=\"updateCorrupted\" placeholder=\"Select\" key=\"key\" label=\"name\">\n        </v-select>\n        <input type=\"hidden\" name=\"corrupted\" v-model=\"corrupted.key\">\n    </div>\n\n    <div class=\"col-lg-4\">\n        <label>Character Name</label>\n        <input type=\"text\" name=\"character\" v-model=\"character\" class=\"form-control\" width=\"265\" style=\"width: 265px; height: 38px;float: left;\">\n    </div>\n\n</div>\n\n<div class=\"row\" style=\"padding-bottom: 40px\">\n\n    <div class=\"col-lg-2\">\n        <label>Sockets</label>\n        <ul class=\"itemLevel\">\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"socket_min\" v-model=\"socket_min\" class=\"num smInputs\" placeholder=\"{{socket_min ? socket_min : 'min'}}\" width=\"50\" style=\"width: 50px\"></li>\n            <li><input type=\"text\" name=\"socket_max\" v-model=\"socket_max\" class=\"num smInputs\" placeholder=\"{{socket_max ? socket_max : 'max'}}\" width=\"50\" style=\"width: 50px\" <=\"\" li=\"\">\n        </li></ul>\n    </div>\n\n    <div class=\"col-lg-3\">\n        <label>Colors</label>\n        <ul class=\"itemLevel\">\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"red_socket\" v-model=\"red_socket\" class=\"socketRed smInputs\" placeholder=\"{{red_socket ? red_socket : 'red'}}\" width=\"50\" style=\"width: 50px\"></li>\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"green_socket\" v-model=\"green_socket\" class=\"socketGreen smInputs\" placeholder=\"{{green_socket ? green_socket : 'green'}}\" width=\"50\" style=\"width: 50px\"></li>\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"blue_socket\" v-model=\"blue_socket\" class=\"socketBlue smInputs\" placeholder=\"{{blue_socket ? blue_socket : 'blue'}}\" width=\"50\" style=\"width: 50px\"></li>\n            <li><input type=\"text\" name=\"white_socket\" v-model=\"white_socket\" class=\"socketWhite smInputs\" placeholder=\"{{white_socket ? white_socket : 'white'}}\" width=\"50\" style=\"width: 50px\"></li>\n        </ul>\n    </div>\n\n    <div class=\"col-lg-3\">\n        <label style=\"alignment-adjust: center\">Exactly</label>\n        <ul class=\"itemLevel\">\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"ex_red_socket\" v-model=\"ex_red_socket\" class=\"socketRed smInputs\" placeholder=\"{{ex_red_socket ? ex_red_socket : 'red'}}\"></li>\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"ex_green_socket\" v-model=\"ex_green_socket\" class=\"socketGreen smInputs\" placeholder=\"{{ex_green_socket ? ex_green_socket : 'green'}}\"></li>\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"ex_blue_socket\" v-model=\"ex_blue_socket\" class=\"socketBlue smInputs\" placeholder=\"{{ex_blue_socket ? ex_blue_socket : 'blue'}}\"></li>\n            <li><input type=\"text\" name=\"ex_white_socket\" v-model=\"ex_white_socket\" class=\"socketWhite smInputs\" placeholder=\"{{ex_white_socket ? ex_white_socket : 'white'}}\"></li>\n        </ul>\n    </div>\n\n    <div class=\"col-lg-2\">\n        <label>Links</label>\n        <ul class=\"itemLevel\">\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"links_min\" v-model=\"links_min\" class=\"num smInputs\" placeholder=\"{{links_min ? links_min : 'min'}}\" width=\"50\" style=\"width: 50px\"></li>\n            <li><input type=\"text\" name=\"links_max\" v-model=\"links_max\" class=\"num smInputs\" placeholder=\"{{links_max ? links_max : 'max'}}\" width=\"50\" style=\"width: 50px\"></li>\n        </ul>\n    </div>\n\n    <div class=\"col-lg-2\">\n        <label>ItemLevel</label>\n        <ul class=\"itemLevel\">\n            <li style=\"padding-right: 5px;\"><input type=\"text\" name=\"level_min\" v-model=\"level_min\" class=\"num smInputs\" placeholder=\"{{level_min ? level_min : 'min'}}\" width=\"50\" style=\"width: 50px\"></li>\n            <li><input type=\"text\" name=\"level_max\" v-model=\"level_max\" class=\"num smInputs\" placeholder=\"{{level_max ? level_max : 'max'}}\" width=\"50\" style=\"width: 50px\" <=\"\" li=\"\">\n        </li></ul>\n    </div>\n\n</div>\n\n<div class=\"row\" style=\"padding-bottom: 15px\">\n    <div class=\"col-lg-3\">\n        <input type=\"checkbox\" name=\"buyout\" class=\"buyout\" v-model=\"buyout\"> Buyout Only<br>\n    </div>\n</div>\n\n<div class=\"mods\">\n    <label>Mods</label>\n    <br><br>\n    <div class=\"mod\" v-for=\"mod in mods\" style=\"padding-bottom: 10px;\">\n        <div class=\"row\">\n            <div class=\"col-lg-8\">\n                <v-select :id=\"mod.id\" :options=\"bigData.mods\" ,=\"\" :selected=\"mods[mod.id].modName\" :show-labels=\"false\" @update=\"updateMod\" placeholder=\"Search for a mod\" label=\"name\" key=\"id\">\n                </v-select>\n                <input type=\"hidden\" name=\"mod[{{mod.id}}][id]\" v-model=\"mods[mod.id].modId\">\n                <input type=\"hidden\" name=\"mod[{{mod.id}}][name]\" v-model=\"mods[mod.id].modName\">\n            </div>\n\n            <div class=\"col-lg-3\">\n                <ul class=\"itemLevel\">\n                    <li style=\"padding-right: 10px;\">\n                        <input style=\"height: 40px;width: 120px; text-align: center\" type=\"text\" name=\"mod[{{mod.id}}][min]\" v-model=\"mods[mod.id].min\" id=\"mod_min\" class=\"num\" placeholder=\"{{mods[mod.id][min] ? mods[mod.id][min] : 'min'}}\">\n                    </li>\n                    <li>\n                        <input style=\"height: 40px;width: 120px; text-align: center\" type=\"text\" name=\"mod[{{mod.id}}][max]\" v-model=\"mods[mod.id].max\" id=\"mod_max\" class=\"num\" placeholder=\"{{mods[mod.id][max] ? mods[mod.id][max] : 'max'}}\">\n                    </li>\n                </ul>\n            </div>\n\n            <div class=\"col-lg-1\">\n                <a href=\"#\" class=\"btn btn-danger btn-sm deleteMod\" style=\"padding-left:10px;\" @click.prevent=\"removeMod(mod)\">Remove</a>\n            </div>\n        </div>\n    </div>\n</div>\n\n<div class=\"row\" style=\"padding-top: 20px;\">\n    <div class=\"col-lg-6 col-lg-offset-5\">\n      <a href=\"#\" class=\"addMod btn btn-primary\" @click.prevent=\"addMod()\">Add Mod</a>\n    </div>\n</div>\n\n<button type=\"submit\" class=\"sub btn btn-default pull-right\" style=\"margin-left: 5px;margin-top: 10px\">Search</button>\n<button class=\"btn btn-danger pull-left reload\" @click.prevent=\"reset()\" style=\"margin-top: 10px\">Reset Search</button>\n\n"
if (module.hot) {(function () {  module.hot.accept()
  ...
  if (!module.hot.data) {
    hotAPI.createRecord("_v-8a1148d2", module.exports)
  } else {
    hotAPI.update("_v-8a1148d2", module.exports, (typeof module.exports === "function" ? module.exports.options : module.exports).template)
  }
})()}
},{"babel-runtime/core-js/object/keys":2,"babel-runtime/helpers/defineProperty":3,"vue":45,"vue-hot-reload-api":42,"vue-multiselect":43,"vueify/lib/insert-css":46}],50:[function(require,module,exports){
var __vueify_insert__ = require("vueify/lib/insert-css")
...
var _Socket = require('./Socket.vue');

var _Socket2 = _interopRequireDefault(_Socket);

var _Price = require('./Price.vue');

var _Price2 = _interopRequireDefault(_Price);

var _Props = require('./Props.vue');
...
exports.default = {
    ...
    props: ['item', 'itemId', 'itemStash'],

    components: { 'socket': _Socket2.default, 'poePrice': _Price2.default, 'props': _Props2.default },
    ...
    data: function data() {
        return {
            ...
            lastGroup: null,
            countSock: 0,
            rarityTypes: [{ key: 0, name: 'Cornsilk' }, { key: 1, name: 'DodgerBlue ' }, { key: 2, name: 'gold' }, { key: 3, name: 'DarkOrange' }, { key: 4, name: 'DarkCyan' }, { key: 5, name: 'yellow' }],
            isClicked: true
        };
    },
    ...
    computed: {
        'fullName': function fullName() {
            return this.item.name + " " + this.item.typeLine;
        },

        'whisper': function whisper() {
            var ifPrice = this.item.note.charAt(0) !== '~' ? ' listed for ' + this.item.note : '';
            return '@' + this.itemStash.lastCharacterName + ' Hi, I would like to buy your ' + this.fullName + ifPrice + ' in Essence (stash tab ' + this.itemStash.stash + '; position: left ' + this.item.x + ', top ' + this.item.y + ')';
        },

        'rarityType': function rarityType() {
            var self = this;
            return this.rarityTypes.filter(function (type) {
                return type.key === self.item.frameType;
            })[0].name;
        },

        'oldMods': function oldMods() {
            if ((0, _keys2.default)(window.PHP.oldFields).length > 0) {
                return window.PHP.oldFields.mod;
            }
            return '';
        }
    },
    ...
    ready: function ready() {
        if (this.item.sockets !== '') {
            this.parseSockets.push(JSON.parse(this.item.sockets));
        }

        if (this.item.implicitMods !== '') {
            this.parseImplict.push(JSON.parse(this.item.implicitMods));
        }

        if (this.item.explicitMods !== '') {
            this.parseExplicit.push(JSON.parse(this.item.explicitMods));
        }

        if (this.item.craftedMods !== '') {
            this.parseCrafted.push(JSON.parse(this.item.craftedMods));
        }

        if (this.item.requirements !== '') {
            this.parseRequirements.push(JSON.parse(this.item.requirements));
        }

        if (this.item.properties !== '') {
            this.parseProperties.push(JSON.parse(this.item.properties));
        }
    },

    methods: {

        findMod: function findMod(mod) {
            var findMod = mod.substr(mod.indexOf(' ') + 1);
            for (var i = 0; i < this.oldMods.length; i++) {
                if (!this.oldMods[i].name.includes(findMod)) {
                    continue;
                }
                return 'red';
            }
        ...
        checkLink: function checkLink(group) {
            var result = this.lastGroup === group;
            this.lastGroup = group;
            return result;
        },

        takeMods: function takeMods() {
            var vm = this;
            var itemObj = {};

            if (vm.item.type !== '') {
                var type = vm.item.type;
                itemObj[type] = [];
                // itemObj.type = vm.item.type

                if (vm.item.explicitMods) {
                    itemObj[type].push(JSON.parse(vm.item.explicitMods));
                }

                if (vm.item.implicitMods) {
                    itemObj[type].push(JSON.parse(vm.item.implicitMods));
                }
            }

            vm.parseStats = itemObj;
        }
...
if (module.exports.__esModule) module.exports = module.exports.default
;(typeof module.exports === "function"? module.exports.options: module.exports).template = "\n<div class=\"row\">\n    <div class=\"col-xs-2\">\n        <div class=\"icon\">\n            <div class=\"sockets\" style=\"position: absolute;\">\n                <div v-if=\"item.sockets\" class=\"socket-inner\" style=\"position: relative; width: 94px;\">\n\n                    <socket v-if=\"item.sockets !== ''\" v-for=\"(index, socket) in parseSockets[0]\" :socket=\"socket\" :current-socket=\"index\" :link=\"checkLink(socket.group)\">\n                    </socket>\n\n                </div>\n            </div>\n            <img style=\"-webkit-user-select: none\" v-bind:src=\"[item.icon]\">\n        </div>\n    </div>\n\n    <div class=\"col-xs-6\">\n        <ul style=\"margin: 0 auto; vertical-align: middle;\">\n            <li class=\"displayInline\" v-if=\"item.corrupted === 1\" style=\"color:red; padding-right: 5px\"> corrupted </li>\n            <li class=\"displayInline\" v-bind:style=\"{ color: rarityType, fontSize: '16px', fontWeight: 'bold' }\">{{ item.name }}</li>\n        </ul>\n\n        <ul style=\"margin: 0 auto; vertical-align: middle; padding-bottom: 5px\" v-if=\"item.requirements !== ''\">\n            <li class=\"displayInline\" v-for=\"req in parseRequirements[0]\" style=\"color: #c1c1c1; inline: block; padding-right: 5px\"> {{ req.name }} : {{ req.values[0][0]}}</li>\n        </ul>\n\n        <ul v-show=\"parseImplict.length > 0\" style=\"margin: 0 auto;\">\n            <li style=\"color: gray; list-style-type: none;border-bottom: 1px groove black;\"> {{ parseImplict[0] }} </li>\n        </ul>\n\n        <ul v-if=\"parseExplicit.length > 0\" style=\"margin: 0 auto;\">\n            <li v-bind:style=\"{listStyleType: 'none', color: findMod(mod)}\" v-for=\"mod in parseExplicit[0]\"> {{ mod }} </li>\n        </ul>\n\n        <ul v-if=\"parseCrafted.length > 0\" style=\"margin: 0 auto;\">\n            <li style=\"list-style-type: none;\" v-for=\"mod in parseCrafted[0]\"> <small style=\"background-color:lightblue\">crafted </small>{{ mod }}</li>\n        </ul>\n\n        <ul style=\"margin: 0 auto; vertical-align: middle; font-weight: bold; padding-top: 10px;\">\n            <poe-price :note=\"item.note\"></poe-price>\n            <li class=\"displayInline\"> @ign:{{itemStash.lastCharacterName}} / </li>\n            <li class=\"displayInline\">  <a href=\"#\" class=\"clipboard\" data-clipboard-text=\"{{whisper}}\" @click=\"changeText\"> {{isClicked ? 'whisper' : 'Copied to clipboard'}}</a> </li>\n        </ul>\n\n    </div>\n\n    <div class=\"col-xs-4 props\">\n        <props :properties=\"parseProperties[0]\" :item-type=\"item.type\"></props>\n    </div>\n\n</div>\n"
if (module.hot) {(function () {  module.hot.accept()
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), true)
  if (!hotAPI.compatible) return
  ...
  if (!module.hot.data) {
    hotAPI.createRecord("_v-3629eb34", module.exports)
  } else {
    hotAPI.update("_v-3629eb34", module.exports, (typeof module.exports === "function" ? module.exports.options : module.exports).template)
  }
})()}
},{"./Price.vue":51,"./Props.vue":52,"./Socket.vue":53,"babel-runtime/core-js/object/keys":2,"vue":45,"vue-hot-reload-api":42,"vueify/lib/insert-css":46}],51:[function(require,module,exports){
'use strict';
...
exports.default = {
    ...
    data: function data() {
        return {
            currencies: [{ name: 'alt', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyRerollMagic.png' }, { name: 'blessed', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyImplicitMod.png' }, { name: 'chance', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyUpgradeRandomly.png' }, { name: 'fuse', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyRerollSocketLinks.png' }, { name: 'alch', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyUpgradeToRare.png' }, { name: 'chaos', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyRerollRare.png' }, { name: 'gcp', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyGemQuality.png' }, { name: 'exa', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyAddModToRare.png' }, { name: 'chrom', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyRerollSocketColours.png' }, { name: 'jew', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyRerollSocketNumbers.png' }, { name: 'chisel', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyMapQuality.png' }, { name: 'scour', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyConvertToNormal.png' }, { name: 'regret ', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyPassiveSkillRefund.png' }, { name: 'regal', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyUpgradeMagicToRare.png' }, { name: 'divine', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyModValues.png' }, { name: 'vaal', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyVaal.png' }, { name: 'mirror', pic: 'http://web.poecdn.com/image/Art/2DItems/Currency/CurrencyDuplicate.png' }]
        };
    },

    computed: {
        priceVal: function priceVal() {
            if (this.note.charAt(0) !== '~') {
                return 0;
            }
            return this.note.split(" ")[1];
        },
        ...
        currencyName: function currencyName() {
            ...
            return this.note.split(" ")[2];
        },

        img: function img() {
            var _this = this;

            var checkImg = this.currencies.filter(function (currency) {
                return currency.name === _this.currencyName;
            })[0];
            return checkImg ? checkImg.pic : '';
        }
    }

};
if (module.exports.__esModule) module.exports = module.exports.default
;(typeof module.exports === "function"? module.exports.options: module.exports).template = "\n<li v-show=\"priceVal > 0\" class=\"displayInline\">\n    {{priceVal}}\n    <small>x</small>\n    <img v-bind:src=\"[img]\" style=\"height:20px; width:20px\"> /\n</li>\n"
if (module.hot) {(function () {  module.hot.accept()
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), true)
  if (!hotAPI.compatible) return
  if (!module.hot.data) {
    hotAPI.createRecord("_v-87fc7ef4", module.exports)
  } else {
    hotAPI.update("_v-87fc7ef4", module.exports, (typeof module.exports === "function" ? module.exports.options : module.exports).template)
  }
})()}
},{"vue":45,"vue-hot-reload-api":42}],52:[function(require,module,exports){
'use strict';
...
exports.default = {
    ...
    props: ['properties', 'itemType'],

    data: function data() {
        return {
            tempQuality: '',
            hasQuality: { status: true },
            tempDmg: null,
            capped: ''
        };
    },
    ...
    computed: {
        'propData': function propData() {
            self = this;
            var backBone = [{ name: "Quality", value: 0 }, { name: "Phys", value: '' }, { name: "Crit", value: '' }, { name: "APS", value: '' }, { name: "DPS", value: '' }, { name: "Armour", value: '' }, { name: "Shield", value: '' }, { name: "Evasion", value: '' }, { name: "Level", value: '' }];
            if (!this.properties) {
                return;
            }

            this.properties.forEach(function (val) {
                if (self.itemType === 'Jewels') {
                    backBone[0].value = '';
                }
                if (val.name === "Quality") {
                    backBone[0].value = self.quality(val.values[0][0]);
                }
                if (val.name === "Physical Damage") {
                    backBone[1].value = self.withQulityWep(val.values[0][0]);
                }
                if (val.name === "Critical Strike Chance") {
                    backBone[2].value = val.values[0][0];
                }
                if (val.name === "Attacks per Second") {
                    backBone[3].value = val.values[0][0];
                    backBone[4].value = (self.tempDmg * val.values[0][0]).toFixed(1);
                }
                if (val.name === "Armour") {
                    backBone[5].value = self.withQulityArmour(val.values[0][0]);
                }
                if (val.name === "Energy Shield") {
                    backBone[6].value = self.withQulityArmour(val.values[0][0]);
                }
                if (val.name === "Evasion Rating") {
                    backBone[7].value = self.withQulityArmour(val.values[0][0]);
                }
                if (val.name === "Level") {
                    backBone[8].value = val.values[0][0];
                }
            });
            backBone = backBone.filter(function (x) {
                return x.value !== '';
            });
            return backBone;
        }
    ...
    methods: {
        quality: function quality(_quality) {
            this.tempQuality = _quality.replace(/[^\d.]/g, '');
            if (this.tempQuality < 20 && this.itemType !== 'Gems') {
                this.capped = '+' + (20 - this.tempQuality);
            }
            return this.tempQuality;
        },

        withQulityWep: function withQulityWep(value) {
            var min = value.split('-')[0];
            min = (min - min * (this.tempQuality / 100) + min * (20 / 100)).toFixed(1);
            var max = value.split('-')[1];
            max = (max - max * (this.tempQuality / 100) + max * (20 / 100)).toFixed(1);
            this.tempDmg = parseFloat(min) + parseFloat(max);
            this.capped = '+' + (20 - this.tempQuality);
            return min + '-' + max;
        },

        withQulityArmour: function withQulityArmour(value) {
            var newVal = '';
            if (this.tempQuality) {
                newVal = value - value * (this.tempQuality / 100) + value * (20 / 100);
                return newVal;
            } else {
                this.capped = '+' + (20 - this.tempQuality);
                newVal = parseFloat(value) + parseFloat(value * (20 / 100));
                return newVal;
            }
        },

        isCapped: function isCapped(currentProp) {
            if (currentProp.name === 'Quality') {
                return true;
            }
            return false;
        }
...
if (module.exports.__esModule) module.exports = module.exports.default
;(typeof module.exports === "function"? module.exports.options: module.exports).template = "\n\n    &nbsp;&nbsp;&nbsp; <table style=\"width:100%\">\n    <tbody><tr><th v-for=\"propNames in propData\">{{propNames.name}}</th>\n    </tr>\n    <tr>\n        <td v-for=\"propVal in propData\">\n            {{propVal.value}}<span v-if=\"isCapped(propVal)\" style=\"color:DarkCyan;\">{{capped}}</span>\n        </td>\n    </tr>\n</tbody></table>\n"
if (module.hot) {(function () {  module.hot.accept()
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), true)
  if (!hotAPI.compatible) return
  if (!module.hot.data) {
    hotAPI.createRecord("_v-105b6d2d", module.exports)
  } else {
    hotAPI.update("_v-105b6d2d", module.exports, (typeof module.exports === "function" ? module.exports.options : module.exports).template)
  }
})()}
},{"vue":45,"vue-hot-reload-api":42}],53:[function(require,module,exports){
'use strict';
...
exports.default = {
    ...
    props: ['socket', 'currentSocket', 'link']

};
if (module.exports.__esModule) module.exports = module.exports.default
;(typeof module.exports === "function"? module.exports.options: module.exports).template = "\n\n<!-- making sure whre to put RightSocket Class -->\n<div v-bind:class=\"['socket socket'+socket.attr, (currentSocket === 2 || currentSocket === 3) ? 'socketRight' : '']\"></div>\n\n<!-- adding div class building the links between socket if there groups match-->\n<div v-if=\"link\" class=\"socketLink socketLink{{currentSocket - 1}}\"></div>\n\n"
if (module.hot) {(function () {  module.hot.accept()
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), true)
  if (!hotAPI.compatible) return
  if (!module.hot.data) {
    hotAPI.createRecord("_v-1fe9dc06", module.exports)
  } else {
    hotAPI.update("_v-1fe9dc06", module.exports, (typeof module.exports === "function" ? module.exports.options : module.exports).template)
  }
})()}
},{"vue":45,"vue-hot-reload-api":42}],54:[function(require,module,exports){
'use strict';

var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol ? "symbol" : typeof obj; };

var _Form = require('./components/main/Form.vue');

var _Form2 = _interopRequireDefault(_Form);

var _Item = require('./components/main/Item.vue');

var _Item2 = _interopRequireDefault(_Item);

var _compereHelper = require('./compereHelper.js');
...
new Vue({
    ...
    data: {
        ...
        markedItems: [],
        itemClass: {
            default: "row list-group-item list-group-poe-item itemtooltip",
            marked: "row list-group-item list-group-poe-item itemtooltip marked"
        }

    },

    computed: {
        'userAcc': function userAcc() {
            return window.PHP.userAcc;
        }
    },

    components: {
        'item': _Item2.default, 'v-select': _vueMultiselect2.default, 'main-form': _Form2.default
    },

    watch: {
        'selectedChar': function selectedChar(val, oldVal) {
            this.character = this.accountCharacters[val];
        }
    },

    ready: function ready() {
        if (!this.userAcc) {
            this.itemClass.default = "row list-group-item list-group-poe-item itemtooltipFalse";
        }
    ...
    methods: {
        ...
        markItem: function markItem(item) {
            if (!this.isMarked(item)) {
                ...
                var id = this.markedItems.findIndex(function (val) {
                    return val.itemId === item.itemId;
                });
                this.markedItems.splice(id, 1);
            }
        },

        isMarked: function isMarked(item) {
            return this.markedItems.some(function (x) {
                return x.itemId === item.itemId;
            });
        },

        removeMarked: function removeMarked(index) {
            this.markedItems.splice(index, 1);
        },

        showProfile: function showProfile(index) {
            this.selectedChar = index;
            this.showChars = false;
        },

        hideProfile: function hideProfile(name) {
            this.character = '';
            this.itemMods = '';
            this.managerResult = '';
            this.showChars = true;
        ...
        showMods: function showMods(item) {
            this.charItems = false;
            this.itemMods = item;
        },
        ...
        compereItem: function compereItem(item) {
            if (_typeof(this.character) !== 'object') {
                return;
            }
            if (item.type in this.character.items) {
                this.showMods(this.character.items[item.type]);
            }

            var difference = new _compereHelper.CompereItemMods();
            difference.setCharacter(this.character.items);
            difference.compereItem(this.formatItem(item));
            this.managerResult = difference.result();
        },
        ...
        formatItem: function formatItem(item) {
            ...
            if (item.type !== '') {
                ...
                if (item.explicitMods) {
                    itemObj.stats.push(JSON.parse(item.explicitMods));
                }
            }
            return itemObj;
        },
        ...
        addCharacters: function addCharacters() {
            ...
            formData.append('accName', vm.userAcc);

            vm.$http.post('/userAccount', formData).then(function (response) {

                if (jQuery.isEmptyObject(response.data)) {
                    $('#app').prepend('<h4 id="wrongName">Wrong profile name, try again</h4>');
                } else {
                    vm.accountCharacters = response.data;
                }
            });
        }
...
},{"./bootstrap":47,"./compereHelper.js":48,"./components/main/Form.vue":49,"./components/main/Item.vue":50,"vue-multiselect":43}]},{},[54]);
